...
import { normalizeMediaType, validateMediaType } from './media-types.js';

describe('normalizeMediaType', () => {
  const asString = 'application/json';
  const asMediaType: MediaType = { type: asString, parameters: {} };

  test('As string', () => expect(normalizeMediaType(asString)).toEqual(asMediaType));
  test('As MediaType', () => expect(normalizeMediaType(asMediaType)).toEqual(asMediaType));
});
